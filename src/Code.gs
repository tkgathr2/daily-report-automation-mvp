...
const USER_PROPERTY_NOTION_TOKEN = 'NOTION_ACCESS_TOKEN';
...
/**
 * ツール設定を取得
 * @returns {Object} {gmail: boolean, notion: boolean}
 */
function getToolSettings() {
  try {
    ...
    if (!dataStr) {
      // デフォルト設定（Gmail ON, Notion ON）
      return { gmail: true, notion: true };
    }
...
/**
 * ツール設定を保存
 * @param {Object} settings - {gmail: boolean, notion: boolean}
 * @returns {boolean} 保存成功/失敗
 */
...
/**
 * カレンダー予定とGmail/Notion履歴を一括取得（V3）
 * @param {string} dateString - 日付文字列
 * @returns {Object} {calendar: string, gmail: Object, notion: Object, errors: Array}
 */
